...
function runScript( commandname ){
    ...
    if( settings.dismiss == "accept" ){
        ...
        for( var j=0; j<selectedItemNames.length; j++ ){
            var theItem = selectedItemNames[ j ]; 										// full item name with folders, separated by backslashes )
...
function decode( astring ){
    ...
    var parsed1 = astring.replace( match1, "&lt;" );
    var match2 = new RegExp( ">", "gi" );
    return parsed1.replace( match2, "&gt;" );
}
function createXML(){
